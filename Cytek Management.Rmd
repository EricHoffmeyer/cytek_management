...
temp_2 <- temp |>
  mutate(clean_names = ifelse(grepl('ri', name, ignore.case = T), 'Ritz', name),
         clean_names = ifelse(grepl('we', name, ignore.case = T), 'Wenjuan', name),
         clean_names = ifelse(grepl('Alex', name, ignore.case = T), 'Alex', name),
         clean_names = ifelse(grepl('Amandip', name, ignore.case = T), 'Amandip', name),
         clean_names = ifelse(grepl('Ashley', name, ignore.case = T), 'Ashley', name),
         clean_names = ifelse(grepl('Clarissa', name, ignore.case = T), 'Clarissa', name),
         clean_names = ifelse(grepl('Craig', name, ignore.case = T), 'Craig', name),
         clean_names = ifelse(grepl('Dallas', name, ignore.case = T), 'Dallas', name),
         clean_names = ifelse(grepl('dejene', name, ignore.case = T), 'Dejene', name), 
         clean_names = ifelse(grepl('DT', name, ignore.case = T), 'Dejene', name)
...
```{r for loop test set up}
for (x in name_df$pattern) {
  mutate(temp, clean_names = ifelse(grepl('ri', name, ignore.case = T), 'Ritz', name))
}
```
...
```{r test function}
temp_2 <- name.cleaner(name_df$pattern, name_df$cleaned)
```
